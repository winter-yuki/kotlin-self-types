...
main.swift:19:14: error: protocol 'B' can only be used as a generic constraint because it has Self or associated type requirements
...
### 1. Resolve `Self` as a return user type for methods and properties

1. Create `ConeSelfType` in `RawFirBuilder` & `DeclarationsConverter`
2. Checks
   1. No class/type parameter with `Self` name in scope
   2. Method return position only
3. Fix subtyping (to make override work)
   1. Suptyping
   2. FirSubstitutionScope

<!-- `FirTypeResolveTransformer` -> `FirSpecificTypeResolverTransformer` -> `FirTypeResolverImpl`
