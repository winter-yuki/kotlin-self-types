...
\begin{definition}
    ...
    Если два типа \texttt{C} и \texttt{D} не свзаны отношением подтипизации, это обозначается так: \texttt{C !<:> D}.
\end{definition}
...
\begin{itemize}
    ...
    \item С поддержкой параметрического полиморфизма (опр.~\ref{def:param-poly}) с ограничениями и вариантности (\ref{subsubsec:variance});
...
\begin{definition}
    \label{def:covariant}
    \term{Ковариантный типовой параметр} --- может использоваться в исходящих позициях деклараций методов класса и как типовой аргумент других типовых параметров в ковариантных позициях\footnote{Для данного изложения можно упрощенно считать, что \term{ковариантные позиции} --- ковариантные параметры типов в исходящей позиции и контравариантные параметры типов во входящей позиции.}.
...
\begin{definition}
    ...
    \term{Контравариантный типовой параметр}~--- может использоваться во входящих позициях деклараций методов класса и как типовой аргумент других типовых параметров в контравариантных позициях.
...
Self-типы в встречаются так же под названием \texttt{This-типы}.
...
Однако, как мы покажем в следующем разделе, Self-типы полезны для нескольких шаблонов программирования.
