...
Даже разрабатывались объектно ориентированные языки, полностью основанные на этом отношении~\cite{bruce1997subtyping}.
...
Другим подходом является введение отдельной вселенной \term{точных типов (exact types)}.

Однако переменная со статическим точным типом $\#T$ может во время исполнения иметь только тип $T$, либо его наследника, но ровно с таким же публичным интерфейсом.
