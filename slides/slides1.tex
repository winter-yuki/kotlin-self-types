...
%    1 - результаты
    ...
    \begin{frame}[fragile]{Рекурсивные дженерики}
        \note<1->{
            0:30 - 1:15
            \\\\
            Система типов языка Котлин недостаточно гибка на данный момент.
            \\\\
            Рассмотрим пример.
            Метод add персистентной коллекции возвращает новую коллекцию с уже добавленным элементом.
            Чтобы вернуть именно тот тип, на котором он был вызван, введём типовой параметр с рекурсивным ограничением.
        }
        \note<2->{
            \\\\
            Этот параметр требуется распространить по всей иерархии наследования.
        }
        \note<3->{
            \\\\
            Чтобы воспользоваться интерфейсом, требуется отдельный типовой параметр, однако тогда add возвращает тип с нужным ограничением.
            \\\\
            ....
        }
    ...
    \begin{frame}[fragile]{Self-типы}
        \note<1->{
            1:45 - 2:15
            \\\\
            У описанной проблемы существует каноническое решение --- Self-типы.
            \\\\
            ....
        }
    ...
    \begin{frame}[fragile]{Решения в других языках}
        \note<1->{
            ...
            ....
            \\\\
            Для объектно-ориентированных языков ситуация сложнее.
            \\\\
            ....
            \\\\
    ...
    \begin{frame}[fragile]{Новые значения Self-типа}
        \note<1->{
            4:00 - 4:30
            \\\\
            Для многих важных приложений, например, персистентных коллекций, необходимо уметь создавать новые объекты Self-типа.
            \\\\
            ....
        }
    ...
    \begin{frame}{Позиции Self-типа}
        \note<1->{
            4:30 - 5:00
            \\\\
            Важнейшую роль в безопасности Self-типов выбор допустимых позиций.
            \\\\
            ....
            \\\\
            Поэтому было решено ограничить использование Self-типов на ковариантные позиции.
            \\\\
            В каждой из них, Self-тип при вызове метода переписывается в тип ресивера, это называется материализацией.
            \\\\
            Таким образом, были описаны правила типизации для Self-типов и далее я реализовал их в компиляторе Kotlin.
        }
        ...
        \begin{block}{Правило материализации}
            Self-тип вызываемого метода всегда переписывается в тип ресивера.
        \end{block}
    \end{frame}
    ...
    \begin{frame}{Реализация Self-типов в компилиторе Kotlin}
        ...
        \begin{block}{Типовой скоуп}
            Множество функций, для которых объект данного типа может быть использован как ресивер: $(plus : String\ldotp(String) \to String) \in scope(String)$.
        \end{block}
    \end{frame}
    ...
    \begin{frame}{Соответствие правил для Kotlin с [\href{https://dl.acm.org/doi/pdf/10.1145/2888392}{Ryu et al, 2016}]}
        ...
        \begin{block}{Материализация}
            Материализация сходна с одним шагом экспансии изорекурсивного типа:
            \vspace{-1em}
            \begin{columns}[onlytextwidth]
