...
\begin{document}
    ...
    \note{
        ...
        \begin{enumerate}
            \item Добрый день, уважаемая комиссия
        ...
        \note{
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    ...
                    \item Поэтому вызов метода add на наследнике возвращает базовый тип
                    \item И попытка вызова метода наследника порождает ошибку типизации несмотря на корректность кода
                \end{enumerate}
            \end{enumerate}
        }
    ...
    \begin{frame}[fragile]{Существующие способы записи типа получателя}
        \note{
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    \item Однако это не очень безопасно из-за явного приведения типов
                ...
                \begin{enumerate}
                    \item Этот вариант не имеет должного контроля со стороны компилятора
    ...
    \begin{frame}[fragile]{Self-типы}
        \note{
            ...
            \begin{enumerate}
                ...
                \end{enumerate}
                \item Так, нужно просто вернуть Self-тип в методе add базового класса
                \begin{enumerate}
                    \item и теперь при вызове add на наследнике сможем на результате вызывать методы наследника
                    \item Таким образом, корректный код протипизируется
                \end{enumerate}
            \end{enumerate}
        }
    ...
    \begin{frame}{Цель и задачи}
        \note{
            ...
            \begin{enumerate}
                ...
                \item Для этого нужно выделить особенности существующих решений, а именно:
                \begin{enumerate}
    ...
    \begin{frame}{Существующие реализации Self-типов}
        \note{
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    \item где на простой модели становятся видны как места, в которых система может стать небезопасной,
                    \item так и возможные решения этих проблем
                \end{enumerate}
                \item Также немало прикладных языков уже имеют поддержку Self-типов
                \begin{enumerate}
...
%                \item Свойства результатов доказываются на модельных исчислениях [\href{https://dl.acm.org/doi/pdf/10.1145/503502.503505}{FJ}, \href{https://dl.acm.org/doi/pdf/10.1145/2888392}{CoreThisJava}]
    ...
    \begin{frame}[fragile]{Переписывание Self-типа}
        \note{
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    \item Self-тип переписывается в объект получателя в его области видимости
        ...
        \note{
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    ...
                    \item А также их непросто использовать из-за множества сопутствующих нетривиальных правил
                \end{enumerate}
                \item Поэтому в прикладных языках не поддерживается типизация посторонних объектов Self'ом
                \item несмотря на наличие важных приложений, таких как персистентные коллекции, например
            \end{enumerate}
        }
    ...
    \begin{frame}[fragile]{Значения Self-типа: решение для Kotlin}
        \note{
            ...
            \begin{enumerate}
                \item Поэтому я сформулировал следующее условие:
                \begin{enumerate}
                    \item Тип C может быть безопасно заменён на Self, если можно показать статически, что он подтип любого возможного типа получателя
                \end{enumerate}
                \item Чтобы проверять его выполнение я ввёл следующее правило:
                \begin{enumerate}
                    \item Класс C должен быть финальныи,
                    \item а тип получателя либо равен C, либо включает его после смарткаста
                    \item Так же должны совпадать модули деклараций, чтобы гарантировать обратную совместимость исходных кодов
                \end{enumerate}
                \item Это правило просто в использовании, как показано на примере справа
...
%                        \item[$+$] Локальное изменение в языке
    ...
    \begin{frame}[fragile]{Контравариантные позиции Self-типа: существующие решения}
        \note{
            ...
            \begin{enumerate}
                ...
                \item Для поддержки сложных методов научные работы вводят массивные изменения в системе типов
                \item либо сильно ограничивают их использование
    ...
    \begin{frame}[fragile]{Контравариантные позиции Self-типа: решение для Kotlin}
        \note{
            ...
            \begin{enumerate}
                \item Я заметил, что для поддержки сложных методов запрещают виртуальную диспетчеризацию на них
                \item поэтому вместо неё можно использовать статическую в виде эмуляции классов типов
                \item это можно сделать через контексты языка Kotlin, как показано на примере
    ...
    \begin{frame}{Результаты}
        \note{
            ...
            \begin{enumerate}
                ...
                \item А так же поддержка Self-типов была реализована в компиляторе за счёт введения специального вида типов и переписывающей Self-типы области видимости
            \end{enumerate}
        }
