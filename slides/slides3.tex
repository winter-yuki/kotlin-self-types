    ...
    \begin{frame}[fragile]{Существующие способы записи типа получателя}
        \note{
            ...
            \begin{enumerate}
                ...
                \item Второй --- добавить переопределяющие методы с более специфичным возвращаемым типом
                \item Третий --- ассоциированные типы, в отличие от предыдущих двух не доступен в Kotlin, а доступен в Scala, например.
            \end{enumerate}
        }
...
%                \item[$-$] Зафиксированный типовой аргумент \texttt{S} не может быть уточнён в наследниках
        ...
        \begin{block}{Ассоциированные типы (Scala)}
            \begin{itemize}
                ...
                \item[$\color{red} -$] Сложность использования и реализации в языке
            \end{itemize}
        \end{block}
    \end{frame}
    ...
    \begin{frame}[fragile]{Self-типы}
        \note{
            ...
            \begin{enumerate}
                ...
                \item И в месте использования на выходе метода add получим нужный тип
            \end{enumerate}
        }
        ...
        \begin{block}{Self-типы уточняют анализ типов}
            \begin{minted}{kotlin}
                ...
                fun test(list: PList<Int>) {
                    list.add(x)         // : #\framebox{PList<Int>}# - тип наследника
        ...
        \note{
            ...
            \begin{enumerate}
                ...
                \begin{enumerate}
                    ...
                    \item В Rust не поддерживаются нетривиальные случаи использования
                    \item Swift же имеет полноценную безопасную поддержку, будем опираться на неё в дальнейшем
                \end{enumerate}
...
%                \item Свойства результатов доказываются на модельных исчислениях [\href{https://dl.acm.org/doi/pdf/10.1145/503502.503505}{FJ}, \href{https://dl.acm.org/doi/pdf/10.1145/2888392}{CoreThisJava}]
    ...
    \begin{frame}[fragile]{Материализация Self-типа}
        \note{
            ...
            \begin{enumerate}
                \item Self-типу небезопасно покидать контекст своего объекта.
                \begin{enumerate}
                    \item В примере если результатом кода в рамке является Self-тип
                    \item То код типизируется
                    \item Таким образом объект типа A на выходе метода unsafe получает область видимости наследника, что приводит к ошибке времени исполнения
                \end{enumerate}
                ...
                \begin{enumerate}
                    ...
                    \item То небезопасный код справа больше не будет типизироваться
                \end{enumerate}
            \end{enumerate}
        }
...
%                \end{block}
    ...
    \begin{frame}[fragile]{Значения Self-типа: существующие решения}
        \note{
            ...
            \begin{enumerate}
                \item Для приложений важно уметь безопасно типизировать другие объекты Self-типом
                \item Академические решения предполагают введение специальных видов методов
                \item Однако это затрудняет развитие языка --- в дальнейшем эти методы придётся специальным образом учитывать
                \item А так же эти возможности непросто использовать из-за нетривиальных правил, сопутствующим этим методам
    ...
    \begin{frame}[fragile]{Значения Self-типа: решение для Kotlin}
        \note{
            ...
            \begin{enumerate}
                \item Я сформулировал следующее условие безопасной типизации посторонних объектов Self-типом
                \item ..
                \item Его выполнение я гарантирую введением следующего правила
                \item ..
                \item Их отличают от рассмотренных выше походов то, что
                \item им просто пользоваться, создаём объект того же класса, в котором находимся
                \item Но взамен имеет ограниченную поддержку открытых классов
            \end{enumerate}
        }

        \begin{block}{Условие безопасной замены типа \texttt{C} на \texttt{Self}}
            \mintinline{kotlin}|C::class isSubtypeOf this.getClass()| должно выполняться статически для места кода
        \end{block}
...
%                        \item[$+$] Локальное изменение в языке
                        \item[$+$] Просто для использования
                        \item[$-$] Ограниченная поддержка открытых классов
                    \end{enumerate}
                \end{block}
            \end{column}\hfill%
            \begin{column}[t]{0.485\textwidth}
    ...
    \begin{frame}[fragile]{Контравариантные позиции Self-типа: существующие решения}
        ...
        \begin{block}{Академические результаты}
            \begin{itemize}
                \item Есть сложный метод $\Rightarrow$ наследник не является подтипом~[\href{https://dl.acm.org/doi/pdf/10.1145/96709.96721}{Cook et al, 1989}]
                \begin{itemize}
        ...
        \begin{block}{Решение в Swift}
            \begin{itemize}
                ...
                \item[$\color{red} -$] Добавление сложного метода нарушает совместимость исходных кодов
%                \item В классах Self-типы в контравариантной позиции запрещены
    ...
    \begin{frame}[fragile]{Контравариантные позиции Self-типа: решение для Kotlin}
        \begin{itemize}
            ...
            \item
            \item
            \item Все решения запрещают на сложных методах
            \item
            \item
            \item Заменим виртуальную диспетчеризацию статической с помощью эмуляции трейтов
            \item Другая возможность языка покрывает сценарии использования сложных методов
        \end{itemize}
